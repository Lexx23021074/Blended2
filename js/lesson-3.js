...
const oneArray1 = data.flatMap(item => item.values);
console.log(oneArray1);

const getArray = (array) => {
  return array.flatMap((number) =>
    number.values);
};
console.log(getArray(data));
//3 
const getArray2 = (array) => {
  return array.reduce((acc, currObj) => {
    const newValues = currObj.values;
  
    return acc.concat(newValues);
  }, []); // [] — це початкове значення для acc
      
};
console.log(getArray2(data));
